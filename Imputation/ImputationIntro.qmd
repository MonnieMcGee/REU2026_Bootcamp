...
library(tidyverse)
library(mice)
library(naniar) # naniar vignette's are ways to go through a case study
library(VIM)
library(ggplot2)
...
```{r mean2}
# Retype the vector y with NA instead of 4 in the last position. Recalculate the mean. What happens?
```
...
```{r mean3}
# Set na.rm = TRUE. What happens?
y <- c(1, 2, NA)
mean(y)
# when mean function sees NA, it just outputs NA

# this means we remove missing values, default is na.rm = FALSE
mean(y, na.rm = TRUE)
```
...
It gets worse with multivariate analysis. For example, let us try to predict daily ozone concentration (ppb) from wind speed (mph) using the built-in `airquality` dataset. We fit a linear regression model by calling the `lm()` function to predict daily ozone levels, as follows:

```{r air2, eval = FALSE}
# Ozone - response/explanatory 
fit <- lm(Ozone ~ Wind, data = airquality)
fit
summary(fit) # tells us 37 observation were deleted due to missingness
```

Many `R` users have seen this message. The code cannot continue because there are missing values. One way to circumvent the problem is to omit any incomplete records by specifying the `na.action = na.omit` argument to `lm()`. The regression weights can now be obtained as

```{r air3}
# Rerun the linear regression with na.action = na.omit. What happens?
# na.action = na.omit was used by default 
```

This works. For example, we may produce diagnostic plots by `plot(fit)` to study the quality of the model. In practice, it is cumbersome to supply the `na.action()` function each time. We can change the setting in `options` as

```{r air3b}
options(na.action = na.omit)
```

which eliminates the error message once and for all. Users of other software packages like `SPSS`, `SAS` and `Stata` enjoy the “luxury” that this deletion option has already been set for them, so the calculations can progress silently.
...
Removing the incomplete cases prior to analysis is known as *listwise deletion* or *complete-case analysis*. In `R`, there are two related functions for the subset of complete cases, `na.omit()` and `complete.cases()`.

Figure \@ref(fig:plotair9) plots the predicted against the observed values. Here we adopt the Abayomi convention for the colors [@ABAYOMI2008]: Blue refers to the observed part of the data, red to the synthetic part of the data (also called the *imputed values* or *imputations*), and black to the combined data (also called the *imputed data* or *completed data*).
...
fit2 <- lm(Ozone ~ Wind, data = airquality,
           na.action = na.exclude)

pred2 <- cbind(1, airquality$Wind) %*% coef(fit2)

abline(h = pred2,
       col = "red", lwd = 0.6, lty = 1)
```
...
```{r airx6}
# 37 plus 7 are 44, but two were missing in both so we have 42 
...
cov_pair <- cov(data, use = "pairwise.complete.obs")
cov_complete <- cov(na.omit(data))

cov_pair
cov_complete

eigen(cov_pair)$values
```

The method is simple, and appears to use all available information. Under MCAR, it produces consistent estimates of mean, correlations and covariances [@LITTLE2002 p. 55]. The method has also some shortcomings. First, the estimates can be biased if the data are not MCAR. Further, the covariance and/or correlation matrix may not be positive definite, which is requirement for most multivariate procedures. Problems are generally more severe for highly correlated variables [@LITTLE1992]. It is not clear what sample size should be used for calculating standard errors. Taking the average sample size yields standard errors that are too small[@LITTLE1992]. Also, pairwise deletion requires numerical data that follow an approximate normal distribution, whereas in practice we often have variables of mixed types.
...
For the airquality data, pairwise deletion seems OK. The covariance matrix looks reasonable. The problem is that we have no guarantee that it will remain reasonable in other datasets.
...
n_pair <- outer(
  ...
  Vectorize(function(x,y)
    sum(complete.cases(airquality[,c(x,y)]))
  )
)

dimnames(n_pair) <- list(vars, vars)
n_pair
```
...
which searches the Comprehensive R Archive Network (CRAN), and installs the requested package on the local computer. After succesful installation, the `mice` package can be loaded by
...
```{r meanimp}
# m - number of datasets you want to create
# 
...
# Mark which Ozone values were originally observed vs imputed
plot_data <- data %>%
  mutate(status = if_else(is.na(airquality$Ozone),
                          "Imputed", "Observed"))

# Histogram: observed vs imputed Ozone values
ggplot(plot_data, aes(x = Ozone, fill = status)) +
...
# Scatterplot: Ozone vs Solar Radiation
ggplot(plot_data, aes(x = Solar.R, y = Ozone, color = status)) +
...
mean(airquality$Ozone, na.rm = TRUE) # original with deleted values
mean(data$Ozone) # data imputed with the mean
# They are the same
```

Mean imputation distorts the distribution in several ways. Figure \@ref(fig:plotmeanimp) displays the distribution of `Ozone` after imputation. In the figure on the left, the red bar at the mean stands out. Imputing the mean here actually creates a bimodal distribution. The standard deviation in the imputed data is equal to 28.7, much smaller than from the observed data alone, which is 33. The figure on the right-hand side shows that the relation between `Ozone` and `Solar.R` is distorted because of the imputations. The correlation drops from 0.35 in the blue points to 0.3 in the combined data.
...
# Yobs = original Ozone values
Yobs <- airquality$Ozone

# Yimp = original Ozone values, with missing Ozone replaced by predicted values
Yimp <- Yobs
Yimp[is.na(Yobs)] <- pred

plot_data <- data.frame(
  Ozone = Yimp,
  Solar.R = airquality$Solar.R,
  status = if_else(is.na(Yobs), "Imputed", "Observed")
) %>%
  filter(!is.na(Solar.R))

# Histogram: observed vs regression-imputed Ozone
ggplot(plot_data, aes(x = Ozone, fill = status)) +
  geom_histogram(binwidth = 10,
                 alpha = 0.45,
                 position = "identity",
...
# Scatterplot: regression-imputed Ozone vs Solar Radiation
ggplot(plot_data, aes(x = Solar.R, y = Ozone, color = status)) +
  geom_point(size = 2, alpha = 0.85) +
  coord_cartesian(ylim = c(-10, 170)) +
  labs(x = "Solar Radiation (lang)",
...
```{r regimp_mea_sd}
# Calculate the mean and SD for the regression imputed values.
```

### Another way to perform regression imputation {#sec:miceregimp}
...
# Histogram of observed vs imputed Ozone
ggplot(plot_data, aes(x = Ozone, fill = status)) +
  geom_histogram(binwidth = 10,
                 alpha = 0.45,
                 position = "identity",
...
The `method = norm.nob` argument requests a plain, non-Bayesian, stochastic regression method. This method first estimates the intercept, slope and residual variance under the linear model, then calculates the predicted value for each missing value, and adds a random draw from the residual to the prediction. We will come back to the details in Section \@ref(sec:linearnormal). The `seed` argument makes the solution reproducible. Figure \@ref(fig:plotsri) shows that the addition of noise to the predictions opens up the distribution of the imputed values, as intended.

Note that some new complexities arise. There is one imputation with a negative value. Such values need not be due to the draws from the residual distribution, but can also be a consequence of the use of a linear model for non-negative data. In fact, Figure \@ref(fig:plotair9) shows several negative predicted values in the observed data. Since negative `Ozone` concentrations do not exist in the real world, we cannot consider negative values as plausible imputations. Note also that the high end of the distribution is not well covered. The observed data form a cone, i.e., the data are heteroscedastic, but the imputation model assumes equal dispersion around the regression line. The variability of `Ozone` increases up to the solar radiation level of 250 langleys, and decreases after that. Though it is unclear whether this is a genuine meteorological phenomenon, the imputation model does not account for this feature.
...
```{r indicator1}
# introduce an indicator variable - missing or not missing
# two separate with and without
...
The solutions are nearly identical here, which is due to the fact that most missing values occur in the outcome variable. The standard errors of the multiple imputation solution are slightly larger than in the complete-case analysis. Multiple imputation is often more efficient than complete-case analysis. Depending on the data and the model at hand, the differences can be dramatic.

```{r plotairmi, echo=FALSE, fig.asp = 0.5, duo = TRUE, fig.cap = '(ref:plotairmi)'}

# Use the first completed data set
                   # can chose any of the other data sets 1, 2...
data <- complete(imp, 1)

# Label values according to whether Ozone was originally missing
plot_data <- data %>%
  mutate(status = if_else(is.na(airquality$Ozone),
                          "Imputed", "Observed"))

# Histogram: observed vs imputed Ozone
ggplot(plot_data, aes(x = Ozone, fill = status)) +
  geom_histogram(binwidth = 10,
                 alpha = 0.45,
                 position = "identity",
...
The computations are done with the help of the `R` package `mice`, written by Karin Groothuis-Oudshoorn and myself [@VANBUUREN2011B]. The book thus also serves as an extended tutorial on the practical application of `mice`. Online materials that accompany the book can be found on www.multiple-imputation.com. My hope is that this hands-on approach will facilitate understanding of the key ideas in multiple imputation.

## What I didn't cover {#sec:doesnotcover}
...
The likelihood-based approach receives an excellent treatment in the book by @LITTLE2002. A less technical account that should appeal to social scientists can be found in @ENDERS2010 [chapters 3–5]. @MOLENBERGHS2007 provide a hands-on approach of likelihood-based methods geared toward clinical studies, including extensions to data that are MNAR. The example of multi likelihood method was introduced by @KATSIKATSOU2012 and has been implemented in `lavaan`.
