...
for(t in 2:n){
  # 0.8 is just a random choice, nothing specific, can change to number between -1 to 1 
...
# jaggedness is noise, but each point is quite strongly related to the only before it
```
...
#dev.off() # everything between the first line and these can be used to make a pdf
par(mfrow = c(1, 1))
```
...
* The ACF (Autocorrelation Function) confirms this: several lags are well outside the confidence bands.
...
library(lmtest)
dwtest(fit_climate)
# null is that the true autocorrelation is 0
```
...
# spurious correlations - they aren't correlated but look correlated because they are both related to time                     
```
...
train |>
  ...
  labs(title = "Future in Light Gray")
```
```{r benchmarks}
fit_benchmarks <- train |>
  model(
    Mean = MEAN(Beer), # these are fpp methods
    Naive = NAIVE(Beer),
    Season_naive = SNAIVE(Beer)
  )
...
# all 3 techniques of confidence interval are non-parametric

# 95% bootstrap confidence interval
quantile(boot_means, c(0.025, 0.975))

hist(airquality$Ozone)
# sampling distribution of the sample mean will follow a normal distribution - CLT
hist(boot_means) 
```
...
```{r loess-spans}
# percentage of data to use around current point
...
kde_df <- purrr::map_dfr(bws, function(bw) {
  d <- density(mtcars$mpg, bw = bw, n = 512) # bandwidth
